...
- **接后台领活**：在网页上拿个配对码填进「设备」页，这台机器就挂到后台了——定时上报心跳、
  定时去领工单、干完回报结果，网页上能看到它在线和干过哪些活

目标形态是一个**执行端**：后台统一派任务，各台电脑上的插件定时领任务、执行、回报结果，不依赖网页开着。
目前工单类型只支持 `echo`（打通链路用），发布、补链接、采数据还没接。
